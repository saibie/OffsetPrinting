...
Finally, we check the efficiency over the number of types.
We see that the efficiency increases as the number of types increases in Figure \ref{fig:LinearFitting}.
For each 82 samples, we calculated the efficiency as the follow formula.
\begin{equation}
	\textrm{Efficiency} = \left[\frac{\textrm{Improvement~Total~Cost}}{\textrm{Previous~Total~Cost}}-1\right]\times 100
\end{equation}
The result of the linear fitting shows that the efficiency and the number of types have a positive correlation.
Thus, we see that the improved algorithms have better results in many number of types.
...
\begin{figure}[h!]
	...
	\caption{The linear fitting for the efficiency over the number of types}
